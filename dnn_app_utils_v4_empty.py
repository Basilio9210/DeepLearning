...
def inicializar_parametros_profundos(layer_dims):
    ...
    np.random.seed(3)
    parameters = {}   # se inicializa parametros para crear un diccionario
...
def linear_forward(A, W, b):
    """
    Implementa la parte lineal de la propagación hacia adelante para una capa cualqiera
...
def linear_activation_forward(A_prev, W, b, activation):
    """
    Implementa la propagación hacia adelante para la capa Z=LINEAR-> A=ACTIVATION (activaciòn de la capa)
    ...
    if activation == "sigmoid":
        ...
        A, activation_cache = sigmoid(Z)                    # A= 1/(1+np.exp(-Z))
        ...
        Z, linear_cache = linear_forward(A_prev, W, b)
        A, activation_cache = relu(Z)                            # A= np.maximum(0,Z)
...
def L_model_forward(X, parameters):
    ...
    for l in range(1, L):
        ...
        caches.append(cache)                #Crea la lista
